# -*- coding: utf-8 -*-
"""
Created on Tue May 25 11:30:37 2021
...
class birds:
    ...
    def __init__(self, textfile):
        ...
        for i in range(len(self.data)):
           ...
           a = self.data[i][0]
           city='Södra Sandby'
           night = city.night_utc(datetime.datetime(self.data[i][0]), local=true)
    ...
    def preprocess(self):
        ...
        for i, element in enumerate(self.data):
            ...
            Dawn[i] = Astral().sun_utc(date = (data[i][0])
                                            .astimezone(
                                            pytz.timezone(
                                            'Europe/Stockholm')),
                                             latitude = 55.72,
                                             longitude=13.35)['dawn']
            Dusk[i] = Astral().sun_utc(date = (data[i][0])
                                            .astimezone(
                                            pytz.timezone(
                                            'Europe/Stockholm')),
                                             latitude = 55.72,
                                             longitude=13.35)['dusk']
        self.Dawn = Dawn
        self.Dusk = Dusk
        print(self.Dawn[0])
        """
        ...
        while (i < len(self.data)+1):
            try:
                 Dawn.append(daytime.sun_utc(date = (self.data[i][0])
                                                .astimezone(
                                                pytz.timezone(
                                                'Europe/Stockholm')),
                                                 latitude = 55.72,
                ...
                pass
            try:
                Dusk.append(daytime.sun_utc(date = (self.data[i][0])
                                                .astimezone(
                                                pytz.timezone(
                                                'Europe/Stockholm')),
                                                 latitude = 55.72,
        ...
        T=self.data[i][0]
        for i in range(len(self.data)):
            if Dawn-T > 0 and Dusk-T < Dawn:
                self.Color[i]=0 #day
            else:
                self.Color[i]=1 #night
        
        print(Color[i][5])
                
        """
        a = Astral()
        b = a.sun_utc(date = (data[0][0])
                                            .astimezone(
                                            pytz.timezone(
                                            'Europe/Stockholm')),
                                             latitude = 55.72,
